...
generate_latex_wrapper(NumberOfIncludeStatements) :-
	...
	format('~n',[]),
	format('\\RequirePackage[pdfview=FitBH,pdfstartview=FitBH]{hyperref}~n',[]),
...
%	format('             pdfview=Fit,pdfstartview=Fit]{hyperref}~n',[]),
	...
	format('~n',[]),
	format('\\definecolor{Blue}{rgb}{0,.5,1}~n',[]),
	format('\\newcommand{\\Wb}[1]{\\textcolor{Blue}{\\textbf{#1}}}~n',[]),
	...
	format('~n',[]),
	format('\\addtolength{\\inferLineSkip}{2pt}~n',[]),
	format('\\newlength{\\wdtotal}~n',[]),
	format('\\newlength{\\httotal}~n',[]),
	...
	format('~n',[]),
	format('\\setlength{\\paperwidth}{2in+\\wdtotal}~n',[]),
...
latex_nd_output(_N,Meaning,rule(Name,A,S,Sem,Rs),Con,Subst,NV) :-
