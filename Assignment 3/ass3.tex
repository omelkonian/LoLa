...
\begin{align*}
& \w{some} \\
=& \ceil{np \otimes n}^\bot \note{Negative Hypothesis} \\
=& (\ceil{np}^\bot \otimes \ceil{n})^\bot \note{ $\ceil{A \otimes B}$ with A and B positive} \\
\\
& \w{popular} \\
=& \ceil{n \otimes n}^\bot \note{Negative Hypothesis}\\
...
\\
& \w{arrived} \\
=& \ceil{np \otimes s}^\bot \note{Negative Hypothesis} \\
=& (\ceil{np} \otimes \ceil{s}^\bot)^\bot \note{$\ceil{A \otimes B}$ with A positive and B negative}\\
\\
& \alpha \\
=& \ceil{s}^\bot \note{Negative Hypohesis} \\
\\
& z \\
=& \ceil{np} \note{Positive Hypothesis}\\
\\
...
\tsc{term} & \tsc{type}\\
\hline
\w{some} & (\ceil{np}^\bot \otimes \ceil{n})^\bot \\
\w{popular} & (\ceil{n}^\bot \otimes \ceil{n})^\bot \\
\w{saint} & \ceil{n} \\
\w{arrived} &  (\ceil{np} \otimes \ceil{s}^\bot)^\bot \\
\alpha & \ceil{s}^\bot \\
z & \ceil{np} \\
y & \ceil{n} \\
\end{array}\]
